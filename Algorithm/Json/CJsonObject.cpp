	...
	bool CJsonObject::AddEmptySubObject(const std::string& strKey)
{
    ...
    cJSON* pJsonSTRUT = cJSON_CreateObject();
    if (pJsonSTRUT == NULL)
    {
        m_strErrMsg = std::string("create sub empty object error!");
        return(false);
    }
    cJSON_AddItemToObject(pFocusData, strKey.c_str(), pJsonSTRUT);
    m_listKeys.clear();
    return(true);
}
	...
	bool CJsonObject::AddEmptySubArray(const std::string& strKey)
{
    ...
    cJSON* pJsonSTRUT = cJSON_CreateArray();
    if (pJsonSTRUT == NULL)
    {
	...
	CJsonObject& CJsonObject::operator[](const std::string& strKey)
{
    ...
    {
        cJSON* pJsonSTRUT = NULL;
        if (m_pJsonData != NULL)
        {
            if (m_pJsonData->type == cJSON_Object)
            {
                pJsonSTRUT = cJSON_GetObjectItem(m_pJsonData, strKey.c_str());
            }
        }
        else if (m_pExternJsonDataRef != NULL)
        {
            ...
            {
                pJsonSTRUT = cJSON_GetObjectItem(m_pExternJsonDataRef, strKey.c_str());
            }
        }
        if (pJsonSTRUT == NULL)
        {
        ...
        {
            CJsonObject* pJsonObject = new CJsonObject(pJsonSTRUT);
            m_mapJsonObjectRef.insert(std::pair<std::string, CJsonObject*>(strKey, pJsonObject));
            return(*pJsonObject);
        }
    }
	...
	CJsonObject& CJsonObject::operator[](unsigned int uiWhich)
{
    ...
    if (iter == m_mapJsonArrayRef.end())
    {
        ...
        if (m_pJsonData != NULL)
        {
            ...
            {
                pJsonSTRUT = cJSON_GetArrayItem(m_pJsonData, uiWhich);
            }
        }
        else if (m_pExternJsonDataRef != NULL)
        {
            ...
            {
                pJsonSTRUT = cJSON_GetArrayItem(m_pExternJsonDataRef, uiWhich);
            }
        }
        if (pJsonSTRUT == NULL)
        {
	...
	std::string CJsonObject::operator()(const std::string& strKey) const
{
    ...
    if (pJsonSTRUT->type == cJSON_String)
    {
        return(pJsonSTRUT->valuestring);
    }
    else if (pJsonSTRUT->type == cJSON_Int)
    {
        char szNumber[128] = {0};
        if (pJsonSTRUT->sign == -1)
        {
            if ((int64)pJsonSTRUT->valueint <= (int64)INT_MAX && (int64)pJsonSTRUT->valueint >= (int64)INT_MIN)
            {
				_snprintf(szNumber, sizeof(szNumber), "%d", (int32)pJsonSTRUT->valueint);
            }
            else
            {
				_snprintf(szNumber, sizeof(szNumber), "%lld", (int64)pJsonSTRUT->valueint);
            }
        }
        else
        {
            if (pJsonSTRUT->valueint <= (uint64)UINT_MAX)
            {
				_snprintf(szNumber, sizeof(szNumber), "%u", (uint32)pJsonSTRUT->valueint);
            }
            else
            {
				_snprintf(szNumber, sizeof(szNumber), "%llu", pJsonSTRUT->valueint);
            }
        }
        return(std::string(szNumber));
    }
    else if (pJsonSTRUT->type == cJSON_Double)
    {
        char szNumber[128] = {0};
        if (fabs(pJsonSTRUT->valuedouble) < 1.0e-6 || fabs(pJsonSTRUT->valuedouble) > 1.0e9)
        {
			_snprintf(szNumber, sizeof(szNumber), "%e", pJsonSTRUT->valuedouble);
        }
        else
        {
			_snprintf(szNumber, sizeof(szNumber), "%f", pJsonSTRUT->valuedouble);
        }
    }
    else if (pJsonSTRUT->type == cJSON_False)
    {
        return(std::string("false"));
    }
    else if (pJsonSTRUT->type == cJSON_True)
    {
	...
	bool CJsonObject::Get(const std::string& strKey, CJsonObject& oJsonObject) const
{
    ...
    char* pJsonString = cJSON_Print(pJsonSTRUT);
	...
	bool CJsonObject::Get(const std::string& strKey, std::string& strValue) const
{
    ...
    strValue = pJsonSTRUT->valuestring;
    return(true);
}
	...
	bool CJsonObject::Get(const std::string& strKey, int32& iValue) const
{
    ...
    if (pJsonSTRUT->type != cJSON_Int)
    {
        return(false);
    }
    iValue = (int32)(pJsonSTRUT->valueint);
    return(true);
}
	...
	bool CJsonObject::Get(const std::string& strKey, uint32& uiValue) const
{
    ...
    uiValue = (uint32)(pJsonSTRUT->valueint);
    return(true);
}
	...
	bool CJsonObject::Get(const std::string& strKey, int64& llValue) const
{
    ...
    llValue = (int64)pJsonSTRUT->valueint;
    return(true);
}
	...
	bool CJsonObject::Get(const std::string& strKey, uint64& ullValue) const
{
    ...
    ullValue = (uint64)pJsonSTRUT->valueint;
    return(true);
}
	...
	bool CJsonObject::Get(const std::string& strKey, bool& bValue) const
{
    ...
    if (pJsonSTRUT->type > cJSON_True)
    {
        return(false);
    }
    bValue = pJsonSTRUT->type;
    return(true);
}
	...
	bool CJsonObject::Get(const std::string& strKey, float& fValue) const
{
    ...
    if (pJsonSTRUT->type != cJSON_Double)
    {
        return(false);
    }
    fValue = (float)(pJsonSTRUT->valuedouble);
    return(true);
}
	...
	bool CJsonObject::Add(const std::string& strKey, const CJsonObject& oJsonObject)
{
    ...
    cJSON* pJsonSTRUT = cJSON_Parse(oJsonObject.ToString().c_str());
    if (pJsonSTRUT == NULL)
    {
        m_strErrMsg = std::string("prase json string error at ") + cJSON_GetErrorPtr();
        return(false);
	...
	bool CJsonObject::Add(const std::string& strKey, const std::string& strValue)
{
    ...
    cJSON* pJsonSTRUT = cJSON_CreateString(strValue.c_str());
    if (pJsonSTRUT == NULL)
    {
        return(false);
    }
	...
	bool CJsonObject::Add(const std::string& strKey, int32 iValue)
{
    ...
    cJSON* pJsonSTRUT = cJSON_CreateInt((uint64)iValue, -1);
    if (pJsonSTRUT == NULL)
    {
        return(false);
    }
	...
	bool CJsonObject::Add(const std::string& strKey, uint32 uiValue)
{
    ...
    cJSON* pJsonSTRUT = cJSON_CreateInt((uint64)uiValue, 1);
    if (pJsonSTRUT == NULL)
    {
        return(false);
    }
	...
	bool CJsonObject::Add(const std::string& strKey, int64 llValue)
{
    ...
    cJSON* pJsonSTRUT = cJSON_CreateInt((uint64)llValue, -1);
    if (pJsonSTRUT == NULL)
    {
        return(false);
    }
	...
	bool CJsonObject::Add(const std::string& strKey, uint64 ullValue)
{
    ...
    cJSON* pJsonSTRUT = cJSON_CreateInt(ullValue, 1);
    if (pJsonSTRUT == NULL)
    {
        return(false);
    }
	...
	bool CJsonObject::Add(const std::string& strKey, bool bValue, bool bValueAgain)
{
    ...
    cJSON* pJsonSTRUT = cJSON_CreateBool(bValue);
    if (pJsonSTRUT == NULL)
    {
        return(false);
    }
	...
	bool CJsonObject::Add(const std::string& strKey, float fValue)
{
    ...
    cJSON* pJsonSTRUT = cJSON_CreateDouble((double)fValue, -1);
    if (pJsonSTRUT == NULL)
    {
        return(false);
    }
	...
	bool CJsonObject::Add(const std::string& strKey, double dValue)
{
    ...
    cJSON* pJsonSTRUT = cJSON_CreateDouble((double)dValue, -1);
    if (pJsonSTRUT == NULL)
    {
        return(false);
    }
	...
	bool CJsonObject::Replace(const std::string& strKey, const CJsonObject& oJsonObject)
{
    ...
    cJSON_ReplaceItemInObject(pFocusData, strKey.c_str(), pJsonSTRUT);
    if (cJSON_GetObjectItem(pFocusData, strKey.c_str()) == NULL)
    {
        return(false);
    }
	...
	bool CJsonObject::Replace(const std::string& strKey, uint64 ullValue)
{
    ...
    cJSON* pJsonSTRUT = cJSON_CreateInt((uint64)ullValue, 1);
    if (pJsonSTRUT == NULL)
    {
        return(false);
    }
	...
	bool CJsonObject::Get(int iWhich, CJsonObject& oJsonObject) const
{
    ...
    if (m_pJsonData != NULL)
    {
        if (m_pJsonData->type == cJSON_Array)
        {
            pJsonSTRUT = cJSON_GetArrayItem(m_pJsonData, iWhich);
        }
    }
    else if (m_pExternJsonDataRef != NULL)
    {
        if(m_pExternJsonDataRef->type == cJSON_Array)
        {
            pJsonSTRUT = cJSON_GetArrayItem(m_pExternJsonDataRef, iWhich);
        }
    }
    if (pJsonSTRUT == NULL)
    {
	...
	bool CJsonObject::Add(const CJsonObject& oJsonObject)
{
    ...
    cJSON_AddItemToArray(pFocusData, pJsonSTRUT);
	...
	bool CJsonObject::AddAsFirst(const CJsonObject& oJsonObject)
{
    ...
    int iArraySizeBeforeAdd = cJSON_GetArraySize(pFocusData);
    cJSON_AddItemToArrayHead(pFocusData, pJsonSTRUT);
    int iArraySizeAfterAdd = cJSON_GetArraySize(pFocusData);
    if (iArraySizeAfterAdd == iArraySizeBeforeAdd)
    {
        return(false);
	...
	bool CJsonObject::Replace(int iWhich, const CJsonObject& oJsonObject)
{
    ...
    cJSON_ReplaceItemInArray(pFocusData, iWhich, pJsonSTRUT);
	...
	bool CJsonObject::Replace(int iWhich, int64 llValue)
{
    ...
    cJSON* pJsonSTRUT = cJSON_CreateInt((uint64)((uint64)llValue), -1);
    if (pJsonSTRUT == NULL)
    {
        return(false);
    }
